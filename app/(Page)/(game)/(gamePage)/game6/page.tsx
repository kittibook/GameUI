...
export default function Game_AnimalMatch() {
  ...
  const createProblems = () => {
      ...
      return [
        {
          id: 1,
          name: "ฝน",
          sound: "https://api.bxok.online/public/mp3/rain.mp3",
        },
        {
          id: 2,
          name: "ลม",
          sound: "https://api.bxok.online/public/mp3/soft.mp3",
        },
        {
          id: 3,
          name: "น้ำไหล",
    ...
    const Problems = setting.game6.Point.map((point: any, index: number) => {
      return {
        id: index + 1,
        name: point.answer,
        sound: config.urlImage + point.url,
      };
    });
  ...
  const playAllAnswersText = async () => {
          ...
          <div className="flex flex-col items-center min-h-screen pt-5 px-4">
            ...
            {currentAnimal && !isGameOver && (
              <>
                ...
                <div className="flex flex-wrap justify-center items-center gap-6 mt-15 max-w-4xl">
                  ...
                  <button
                    onClick={playAllAnswersText}
                    className="bg-blue-50 px-4 py-2 rounded-xl shadow text-center w-fit mx-auto mt-6 hover:bg-blue-100 transition"
                  >
                    <p className="text-gray-700 font-mali text-lg md:text-2xl font-bold">
                      ฟังเสียงคำตอบทุกคำตอบ
                    </p>
                  </button>
                </div>
              </>
